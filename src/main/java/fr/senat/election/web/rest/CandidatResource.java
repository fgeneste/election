...
/**
 * REST controller for managing {@link fr.senat.election.domain.Candidat}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CandidatResource {
    ...
    public ResponseEntity<List<Candidat>> getAllCandidats(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("association-is-null".equals(filter)) {
            log.debug("REST request to get all Candidats where association is null");
            return new ResponseEntity<>(StreamSupport
                .stream(candidatRepository.findAll().spliterator(), false)
                .filter(candidat -> candidat.getAssociation() == null)
